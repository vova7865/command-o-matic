...
public class CommandExtendedClone extends CommandBase {
	/**
	...
	public void execute(MinecraftServer server, ICommandSender sender, String[] args) throws CommandException {
		if (args.length < 9) {
			...
			sender.setCommandStat(net.minecraft.command.CommandResultStats.Type.AFFECTED_BLOCKS, 0);
			...
			StructureBoundingBox structureboundingbox1 = new StructureBoundingBox(blockpos2,
					blockpos2.add(structureboundingbox.getLength()));
			...
			if ((args.length < 11 || !"force".equals(args[10]) && !"move".equals(args[10]))
				...
				if ((!world.isOutsideBuildHeight(blockpos)) && (!world.isOutsideBuildHeight(blockpos1))
						&& (!world.isOutsideBuildHeight(blockpos2))
						&& (!world.isOutsideBuildHeight(blockpos2.add(structureboundingbox.getLength())))) {
					if (world.isAreaLoaded(structureboundingbox) && world.isAreaLoaded(structureboundingbox1)) {
						...
						List<CommandExtendedClone.StaticCloneData> list = Lists.<CommandExtendedClone.StaticCloneData>newArrayList();
						List<CommandExtendedClone.StaticCloneData> list1 = Lists.<CommandExtendedClone.StaticCloneData>newArrayList();
						List<CommandExtendedClone.StaticCloneData> list2 = Lists.<CommandExtendedClone.StaticCloneData>newArrayList();
							...
							for (int k = structureboundingbox.minY; k <= structureboundingbox.maxY; ++k) {
								for (int l = structureboundingbox.minX; l <= structureboundingbox.maxX; ++l) {
									...
									if ((!flag1 || iblockstate.getBlock() != Blocks.AIR)
											&& (block == null || iblockstate.getBlock() == block
													&& (predicate == null || predicate.apply(iblockstate)))) {
										TileEntity tileentity = world.getTileEntity(blockpos4);

										if (tileentity != null) {
											NBTTagCompound nbttagcompound = tileentity.writeToNBT(new NBTTagCompound());
											list1.add(new CommandExtendedClone.StaticCloneData(blockpos5, iblockstate,
													nbttagcompound));
											deque.addLast(blockpos4);
										} else if (!iblockstate.isFullBlock() && !iblockstate.isFullCube()) {
											list2.add(new CommandExtendedClone.StaticCloneData(blockpos5, iblockstate,
													(NBTTagCompound) null));
											deque.addFirst(blockpos4);
										} else {
											list.add(new CommandExtendedClone.StaticCloneData(blockpos5, iblockstate,
													(NBTTagCompound) null));
											deque.addLast(blockpos4);
										}
									}
						...
						List<CommandExtendedClone.StaticCloneData> list3 = Lists.<CommandExtendedClone.StaticCloneData>newArrayList();
						list3.addAll(list);
						list3.addAll(list1);
						list3.addAll(list2);
						List<CommandExtendedClone.StaticCloneData> list4 = Lists.<CommandExtendedClone.StaticCloneData>reverse(
								list3);

						for (CommandExtendedClone.StaticCloneData commandclone$staticclonedata : list4) {
						...
						for (CommandExtendedClone.StaticCloneData commandclone$staticclonedata1 : list3) {
							if (world.setBlockState(commandclone$staticclonedata1.pos,
									commandclone$staticclonedata1.blockState, 2)) {
								++i;
							}
						}

						for (CommandExtendedClone.StaticCloneData commandclone$staticclonedata2 : list1) {
							...
							if (commandclone$staticclonedata2.nbt != null && tileentity3 != null) {
								commandclone$staticclonedata2.nbt.setInteger("x",
										commandclone$staticclonedata2.pos.getX());
								commandclone$staticclonedata2.nbt.setInteger("y",
										commandclone$staticclonedata2.pos.getY());
								commandclone$staticclonedata2.nbt.setInteger("z",
										commandclone$staticclonedata2.pos.getZ());
								tileentity3.readFromNBT(commandclone$staticclonedata2.nbt);
								tileentity3.markDirty();
							}

							world.setBlockState(commandclone$staticclonedata2.pos,
									commandclone$staticclonedata2.blockState, 2);
						}

						for (CommandExtendedClone.StaticCloneData commandclone$staticclonedata3 : list4) {
							world.notifyNeighborsRespectDebug(commandclone$staticclonedata3.pos,
									commandclone$staticclonedata3.blockState.getBlock(), false);
						}
						...
						if (i <= 0) {
							...
							sender.setCommandStat(net.minecraft.command.CommandResultStats.Type.AFFECTED_BLOCKS, i);
	...
	public List<String> getTabCompletions(MinecraftServer server, ICommandSender sender, String[] args,
		...
		if (args.length > 0 && args.length <= 3) {
			...
			return args.length == 12 && "filtered".equals(args[9])
