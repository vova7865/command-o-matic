...
public class CommandExtendedFill extends CommandBase implements ICommand {
	/**
	...
	public void execute(MinecraftServer server, ICommandSender sender, String[] args) throws CommandException {
		if (args.length < 7) {
			...
			int i = (blockpos3.getX() - blockpos2.getX() + 1) * (blockpos3.getY() - blockpos2.getY() + 1)
						...
						for (int j1 = blockpos2.getX(); j1 <= blockpos3.getX(); ++j1) {
							...
							if (args.length >= 9) {
								if (!"outline".equals(args[8]) && !"hollow".equals(args[8])) {
									...
									} else if ("replace".equals(args[8]) && !block.hasTileEntity(iblockstate)
											&& args.length > 9) {
										Block block1 = CommandBase.getBlockByText(sender, args[9]);

										if (world.getBlockState(blockpos4).getBlock() != block1
												|| args.length > 10 && !"-1".equals(args[10]) && !"*".equals(args[10])
														&& !CommandBase
																.convertArgToBlockStatePredicate(block1, args[10])
																.apply(world.getBlockState(blockpos4))) {
											continue;
										}
									}
								} else if (j1 != blockpos2.getX() && j1 != blockpos3.getX() && i1 != blockpos2.getY()
										&& i1 != blockpos3.getY() && l != blockpos2.getZ() && l != blockpos3.getZ()) {
	...
	public List<String> getTabCompletions(MinecraftServer server, ICommandSender sender, String[] args,
		...
		if (args.length > 0 && args.length <= 3) {
			...
			return args.length == 10 && "replace".equals(args[8])
